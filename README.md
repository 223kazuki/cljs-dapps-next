...
* [`src/cljs/cljs_dapps_next/`](src/cljs/cljs_dapps_next/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/cljs_dapps_next/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/cljs_dapps_next/`](test/cljs/cljs_dapps_next/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `cljs-dapps-next` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/cljs_dapps_next/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns cljs-dapps-next.example
  (:require [cljs-dapps-next.config :as config])
